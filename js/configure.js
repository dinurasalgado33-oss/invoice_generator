...
function renderVillaList() {
  ...
  list.innerHTML = rooms.map((room, i) => `
    <tr class="list-item-row">
      ...
      <td>
        <button type="button" class="list-edit-btn" data-index="${i}" aria-label="Edit rate for ${escapeHtml(room.name)}">
  ...
  list.querySelectorAll(".list-edit-btn").forEach(btn => {
    btn.addEventListener("click", () => openVillaRateSheet(Number(btn.dataset.index)));
  });
}

function openVillaRateSheet(index) {
  const room = ROOMS_BY_BRANCH[appState.selectedBranch][index];
  editingVillaIndex = index;
...
document.getElementById("villa-rate-form").addEventListener("submit", (e) => {
  e.preventDefault();
  if (editingVillaIndex === null) return;
  ...
  const room = ROOMS_BY_BRANCH[appState.selectedBranch][editingVillaIndex];
  room.rate = rate;
